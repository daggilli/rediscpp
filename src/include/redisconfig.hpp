...
namespace RedisCpp {
   ...
   * * `useResp3` Whether the RESP3 reply format should be used (default false, meaning use RESP2)
   */
  class Config {
    friend class Client;
   ...
   public:
    explicit Config(const std::string &hostname_, int port_, int db_ = 0,
                    ...
                    std::optional<std::string> pw_ = std::nullopt, std::optional<bool> ur3_ = std::nullopt)
   ...
   private:
    ...
    friend std::string text(Config const &cfg) {
      return std::format(
          ...
          cfg.password.value_or("nil"), cfg.useResp3.value_or(false));
    }
